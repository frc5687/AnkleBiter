...
public class TestMotor extends OutliersSubsystem {
    ...
    public void updateDashboard() {
        metric("Current Draw Good", _goodTalon.getSupplyCurrent().getValue().in(Units.Amps));
        metric("Current Draw Bad", _badTalon.getSupplyCurrent().getValue().in(Units.Amps));
        metric("RPM Good Motor", _goodTalon.getRotorVelocity().getValue().in(Units.RPM));
        metric("RPM Bad Motor", _badTalon.getRotorVelocity().getValue().in(Units.RPM));
    }
